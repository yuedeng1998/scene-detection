...
import numpy as np
from util import load, build_vocabulary, get_bags_of_sifts,plo_class_histg
from classifiers import nearest_neighbor_classify, svm_classify,plot_confusion_matrix,getclasses
...
kmeans = build_vocabulary(train_image_paths, vocab_size=300)
# pickle.dump(kmeans, open('kmeans.pkl', 'wb'))
#kmeans = pickle.load(open('kmeans.pkl', 'rb'))
#TODO: You code get_bags_of_sifts function in util.py 
train_image_feats = get_bags_of_sifts(train_image_paths, kmeans)
# pickle.dump(train_image_feats, open('tf.pkl', 'wb'))
#train_image_feats = pickle.load(open('tf.pkl', 'rb'))
# import pdb; pdb.set_trace()
plo_class_histg(train_image_feats, train_labels, kmeans)
test_image_feats = get_bags_of_sifts(test_image_paths, kmeans)
# pickle.dump(test_image_feats, open('ttf.pkl', 'wb'))
# test_image_feats = pickle.load(open('ttf.pkl', 'rb'))
plo_class_histg(test_image_feats, test_labels, kmeans)
...
print(accuracy_score(test_labels, pred_labels_knn))
print(accuracy_score(test_labels, pred_labels_svm))
# cm = confusion_matrix(test_labels,pred_labels_knn)
# pl.matshow(cm)
# pl.title('Confusion knn matrix of the classifier')
# pl.colorbar()
# pl.show()
classes = getclasses("sift/test")


plot_confusion_matrix(test_labels,pred_labels_knn, classes = classes)
plt.show()
plot_confusion_matrix(test_labels,pred_labels_svm, classes = classes)
plt.show()
